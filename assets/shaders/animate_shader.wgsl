...
struct Mouse {
    normalised_device_coordinates: vec2<f32>,
}
@group(3) @binding(0)
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    let speed = 2.0;
    let t_1 = sin(time.time_since_startup * speed * mouse.normalised_device_coordinates.x) * 0.5 + 0.5;
    let t_2 = cos(time.time_since_startup * speed * mouse.normalised_device_coordinates.y);
